...
function generateMap(width: number = 32, height: number = 32): Record<string,string> {
    ...
    for (let x = -halfW; x < halfW; x++) {
        for (let y = 0; y < height; y++) {
            let type = 'dirt';
            if (y === 0) type = 'grass';
            else if (y < 5) type = 'dirt';
            else if (y < 10) type = 'stone';
            else if (y < 15) type = Math.random() < 0.1 ? 'ore' : 'stone';
            else type = Math.random() < 0.05 ? 'ore' : 'stone';
            map[`${x},${y}`] = type;
        }
    }
    return map;
...
app.post('/api/session/join', async (c) => {
    ...
    const exists = state.players.find((p: any) => p.name === name);
    if (!exists) {
...
app.post('/api/session/action', async (c) => {
    ...
    if (action.type === 'mine') {
        ...
        // Check player exists
        const player = state.players.find((p: any) => p.name === name);
        if (!player) {
        ...
        (state.chat as any[]).push({ name, message, timestamp: Date.now() });
    }
    await db.update(sessionsTable).set({ state: JSON.stringify(state) }).where(eq(sessionsTable.code, code));
    return c.json({ state });
});
